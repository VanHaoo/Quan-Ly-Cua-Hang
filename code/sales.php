...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if (!$product || (int) $product['stock'] < $quantity) {
            ...
            if ($currentQuantity + $quantity > (int) $product['stock']) {
                ...
                $_SESSION['cart'][$productId] = ['id'=>(int)$product['id'],'code'=>(string)$product['code'],'name'=>(string)$product['name'],'price'=>(float)$product['price'],'quantity'=>$currentQuantity+$quantity];
    ...
    if ($action === 'checkout') {
        try {
            $result = checkout_order($pdo, $_SESSION['cart'], $_POST, (int) current_user()['id'], (string) $_SESSION['checkout_token']);
...
<div class="sales-layout">
    <section class="panel">
        <div class="panel-heading responsive"><div><h2>Chọn sản phẩm</h2><p>Tìm kiếm, quét mã hoặc thêm sản phẩm vào giỏ</p></div><form method="get" class="filter-form"><input name="q" value="<?= e($q) ?>" placeholder="Nhập mã, tên hoặc danh mục"><button class="btn secondary">Tìm</button></form></div>
    ...
    <aside class="panel cart-panel">
        <div class="panel-heading"><div><h2>Giỏ hàng & thanh toán</h2><p><?= count($cart) ?> loại sản phẩm</p></div></div>
        <div class="cart-list">
        ...
        <form method="post" class="form-grid one-column" id="checkout-form">
            ...
            <div class="member-section">
                ...
                <label>Số điện thoại thành viên<div class="inline-field"><input id="customer-phone" name="customer_phone" inputmode="numeric" maxlength="11" placeholder="Ví dụ 0901234567"><button type="button" class="btn secondary" id="lookup-customer">Kiểm tra</button></div></label>
                ...
                <label>Mã voucher<input id="voucher-code" name="voucher_code" list="voucher-list" placeholder="Không bắt buộc" autocomplete="off"><datalist id="voucher-list"></datalist></label>
            </div>
            <label>Phương thức thanh toán<select name="payment_method"><option value="cash">Tiền mặt</option><option value="transfer">Chuyển khoản</option><option value="qr">QR</option></select></label>
            ...
            <button class="btn primary full" type="submit" <?= !$cart ? 'disabled' : '' ?>>Thanh toán và lập hóa đơn</button>
        </form>
...
(() => {
    ...
    async function lookupCustomer() {
        ...
        vouchers.clear(); voucherList.innerHTML = ''; voucherInput.value = ''; refreshPayment();
        if (!phone) { customerInfo.textContent = 'Khách lẻ không tích điểm và không sử dụng voucher.'; return; }
        customerInfo.textContent = 'Đang kiểm tra khách hàng...';
        try {
            const response = await fetch('api/customer_lookup.php', {method:'POST', headers:{'Accept':'application/json','Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','X-CSRF-Token': <?= json_encode(csrf_token()) ?>}, body:new URLSearchParams({phone})});
            const data = await response.json();
            if (!data.found) { customerInfo.textContent = data.message; nameInput.value = ''; return; }
            ...
            data.vouchers.forEach(voucher => {
                ...
                const valueText = voucher.discount_type === 'percent' ? `${Number(voucher.discount_value)}%` : formatMoney(voucher.discount_value);
                option.value = voucher.code;
                option.label = `${voucher.name} · Giảm ${valueText} · Đơn từ ${formatMoney(voucher.min_order)}`;
                voucherList.appendChild(option);
            });
            if (!data.vouchers.length) customerInfo.textContent += ' · Chưa có voucher khả dụng';
        } catch (error) { customerInfo.textContent = 'Không thể tra cứu khách hàng. Bạn vẫn có thể tiếp tục thanh toán.'; }
    }
    ...
    refreshPayment();
    document.getElementById('checkout-form')?.addEventListener('submit', function () { const button = this.querySelector('button[type="submit"]'); button.disabled = true; button.textContent = 'Đang xử lý...'; });
})();
</script>
<?php render_footer(); ?>
