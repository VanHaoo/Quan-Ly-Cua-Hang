...
function new_voucher_code(PDO $pdo): string
{
    ...
    throw new RuntimeException('Không thể tạo mã voucher mới. Vui lòng thử lại.');
}

/**
 * @param array<int, array{id:int,code:string,name:string,price:float,quantity:int}> $cart
 * @param array<string, mixed> $input
 * @return array{invoice_id:int, invoice_code:string, total_amount:float, message:string}
 */
function checkout_order(
    ...
    if ($postedToken === '' || !hash_equals($expectedToken, $postedToken)) {
        throw new RuntimeException('Giao dịch đã được xử lý hoặc phiên thanh toán không hợp lệ.');
    }

    if ($customerPhone !== '' && !preg_match('/^[0-9]{9,11}$/', $customerPhone)) {
        throw new RuntimeException('Số điện thoại phải gồm từ 9 đến 11 chữ số.');
    }
    ...
    try {
        ...
        foreach ($cart as $cartItem) {
            $stmt = $pdo->prepare(
                'SELECT id, code, name, price, stock, status
                 FROM products
                 WHERE id = ?
                 FOR UPDATE'
            );
            ...
            if (!$product || (int) $product['status'] !== 1 || (int) $product['stock'] < $quantity) {
                throw new RuntimeException(
                    'Sản phẩm ' . ($cartItem['name'] ?? '') . ' không còn đủ số lượng.'
                );
            }
        ...
        if ($priceChanges) {
            throw new RuntimeException(
                'Giá sản phẩm vừa thay đổi. Giỏ hàng đã được cập nhật: '
                . implode('; ', $priceChanges)
        ...
        if ($customerPhone !== '') {
            ...
            if ($customer) {
                ...
                if ($customerNameInput !== '' && $customerNameInput !== $customer['name']) {
                    $updateName = $pdo->prepare('UPDATE customers SET name = ? WHERE id = ?');
                    $updateName->execute([$customerNameInput, $customerId]);
                }
            } else {
                if ($customerNameInput === '') {
                    throw new RuntimeException('Khách hàng mới cần nhập họ tên.');
                }

                $stmt = $pdo->prepare('INSERT INTO customers (name, phone) VALUES (?, ?)');
                $stmt->execute([$customerNameInput, $customerPhone]);
        ...
        if ($voucherCode !== '') {
            ...
            if (!$voucher) {
                throw new RuntimeException('Voucher không tồn tại hoặc không thuộc khách hàng này.');
            }
            if ($voucher['status'] !== 'available') {
                throw new RuntimeException('Voucher đã được sử dụng hoặc không còn hiệu lực.');
            }
            if (strtotime((string) $voucher['expires_at']) < time()) {
                throw new RuntimeException('Voucher đã hết hạn.');
            }
            if ($subtotalAmount < (float) $voucher['min_order']) {
        ...
        $pointsEarned = $customerId !== null
            ? (int) floor($totalAmount / 10000)
            : 0;
        ...
        if ($voucherId !== null) {
            $stmt = $pdo->prepare(
                "UPDATE vouchers
                 SET status = 'used', used_at = NOW(), used_invoice_id = ?
                 WHERE id = ?"
            );
            $stmt->execute([$invoiceId, $voucherId]);
        }
        ...
        if ($customerId !== null && $customer) {
            ...
            $stmt->execute([$remainingPoints, $newTotalSpent, $customerId]);
            ...
            for ($index = 0; $index < $voucherCount; $index++) {
                ...
                $stmt = $pdo->prepare(
                    "INSERT INTO vouchers (
                        ...
                        source_invoice_id,
                        points_cost
                     ) VALUES (?, ?, ?, 'fixed', 20000, 100000,
                         DATE_ADD(NOW(), INTERVAL 30 DAY), 'available', ?, 100)"
                );
                $stmt->execute([
                    $customerId,
                    $rewardCode,
                    'Voucher thành viên giảm 20.000 đ',
                    $invoiceId,
                ]);
        ...
        if ($pointsEarned > 0) {
            $message .= ' Khách hàng được cộng ' . $pointsEarned . ' điểm.';
        }
        if ($issuedVouchers) {
            $message .= ' Voucher mới: ' . implode(', ', $issuedVouchers) . '.';
        }
